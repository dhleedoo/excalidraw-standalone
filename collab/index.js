...
try {
    ...
    io_1.on("connection", function (socket) {
        ...
        socket.on("user-follow", function (payload) { return __awaiter(void 0, void 0, void 0, function () {
            var roomID, _a, sockets, followedBy, sockets, followedBy;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        roomID = "follow@" + payload.userToFollow.socketId;
                        _a = payload.action;
                        switch (_a) {
                            case "FOLLOW": return [3 /*break*/, 1];
                            case "UNFOLLOW": return [3 /*break*/, 4];
                        }
                        return [3 /*break*/, 7];
                    case 1: return [4 /*yield*/, socket.join(roomID)];
                    case 2:
                        _b.sent();
                        return [4 /*yield*/, io_1.in(roomID).fetchSockets()];
                    case 3:
                        sockets = _b.sent();
                        followedBy = sockets.map(function (socket) { return socket.id; });
                        io_1.to(payload.userToFollow.socketId).emit("user-follow-room-change", followedBy);
                        return [3 /*break*/, 7];
                    case 4: return [4 /*yield*/, socket.leave(roomID)];
                    case 5:
                        _b.sent();
                        return [4 /*yield*/, io_1.in(roomID).fetchSockets()];
                    case 6:
        ...
        socket.on("disconnecting", function () { return __awaiter(void 0, void 0, void 0, function () {
            var _i, _a, roomID, otherClients, isFollowRoom, socketId;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        socketDebug(socket.id + " has disconnected");
                        _i = 0, _a = Array.from(socket.rooms);
                        _b.label = 1;
                    case 1:
                        ...
                        return [4 /*yield*/, io_1.in(roomID).fetchSockets()];
                    case 2:
                        otherClients = (_b.sent()).filter(function (_socket) { return _socket.id !== socket.id; });
                        isFollowRoom = roomID.startsWith("follow@");
                        if (!isFollowRoom && otherClients.length > 0) {
                            socket.broadcast.to(roomID).emit("room-user-change", otherClients.map(function (socket) { return socket.id; }));
                        }
                        if (isFollowRoom && otherClients.length === 0) {
                            socketId = roomID.replace("follow@", "");
                            io_1.to(socketId).emit("broadcast-unfollow");
                        }
                        _b.label = 3;
                    case 3:
                        _i++;
